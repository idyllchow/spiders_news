...
class NYSpider(scrapy.Spider):
    ...
    def parse_news_dual(self, response):
        ...
        item['image_urls'] = data.xpath("//img[@class='img-lazyload']/@data-url").extract()
        # item['date'] = data.xpath("//meta[@name='date']/@content").extract_first()
        item['date_cn'] = data.xpath("//div[@class='chinese']/div[@class=kickerBox]/span[@class='date']/text()").extract()
        item['date_en'] = data.xpath("//div[@class='english article_en']/div[@class=kickerBox]/span[@class='date']/text()").extract()
